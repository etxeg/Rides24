...
public class CreateRideGUI extends JFrame {
	...
	private JComboBox<Car> carComboBox = new JComboBox<Car>();
	...
	public CreateRideGUI(Driver driver) {
		...
		jLabelCar.setBounds(new Rectangle(6, 119, 173, 20));
		carComboBox.setBounds(new Rectangle(100, 119, 130, 20));
		...
		this.getContentPane().add(carComboBox, null);
		...
		for (Car actualCar : driver.getCars()){
			System.out.println(actualCar.getMarka());
        	carComboBox.addItem(actualCar);
        }
...
//			
	...
	private String field_Errors() {
		...
		try {
			if ((fieldOrigin.getText().length()==0) || (fieldDestination.getText().length()==0) || (carComboBox.getItemCount()==0) || (jTextFieldPrice.getText().length()==0))
